...
public final class ElectricityGridHandler extends AbstractGridHandler<ETileEntityLink> {
    ...
    protected void internalAdd(ETileEntityLink o) {
        ...
        Collection<CircuitElement<?>> elm = CircuitElementFactory.INSTANCE.wrapComponent(tile.getCapability(TechnicalitiesAPI.getElectricityCap(), null));
        elm.forEach(element -> {
            map.put(o, element);
            addElmImpl(element);
        });
    ...
    public boolean isValidObject(TileEntity tile) {
        if (!tile.hasCapability(TechnicalitiesAPI.getElectricityCap(), null)) {
            return false;
        }
        IElectricityDevice eObj = tile.getCapability(TechnicalitiesAPI.getElectricityCap(), null);
    ...
    public void worldUnload(WorldEvent.Unload event) {
        if (!event.getWorld().isRemote && WorldHelper.getDimID(event.getWorld()) == 0) {
            Technicalities.getElectricityGridHandler().clear();
        }
    }
